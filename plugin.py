...
class PluginConfig(ConfigBase):
    ...
    MEM0_API_KEY: str = Field(
        default="",
        title="Mem0 API Key",
        description="Mem0服务的API密钥",
    )

    MEM0_BASE_URL: str = Field(
        default="",
        title="Mem0 Base URL",
        description="Mem0服务的基础URL（可选）",
    )
    ...
    QDRANT_API_KEY: str = Field(
        ...
        description="Qdrant的API密钥（可选）",
    )

    CHROMA_PATH: str = Field(
        default="./chroma_db",
        title="Chroma DB路径",
        description="Chroma数据库存储路径",
    )
    ...
    PRE_SEARCH_ENABLED: bool = Field(
        default=True,
        title="预搜索启用",
        description="是否启用预搜索功能",
    )

    PRE_SEARCH_MESSAGE_COUNT: int = Field(
        default=50,
        title="预搜索消息数量",
        description="预搜索时获取的历史消息数量",
    )
